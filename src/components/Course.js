...
class Course extends Component {
    ...
    render() {
        ...
        const {exChangeValues, exChangeTo} = this.props;
        return (
            <div className="course-card col-12 col-md-6 my-2">
                <div className="card border-0 shadow-sm bg-light">
                    ...
                    <div className="card-footer bg-transparent border-0 clearfix">
                        ...
                        <div className="float-right">
                            {this.state.show ?
                                <button className={`btn btn-outline-danger btn-sm ${this.state.show ? 'show' : ''}`}
                                        onClick={() => this.handleRemoveFromCart(course.id)}>
                                    <FontAwesomeIcon icon={faTrashAlt}/>
                                </button>
                                :
                                <button
                                    className={`btn btn-outline-primary btn-sm ${this.state.show ? '' : 'show'}`}
                                    onClick={() => this.handleAddToCart(course)}>
